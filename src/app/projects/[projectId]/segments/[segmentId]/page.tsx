  ...
  const bottomNav = (
    <div className="mt-3 flex items-center justify-between gap-2 text-xs">
      ...
      <Link
        ...
        aria-label={t.segment.backToProjectAria}
        className="inline-flex items-center gap-1 rounded-chip border border-ink-line bg-paper px-3 py-2 font-medium text-ink-muted transition hover:border-ink hover:text-ink"
      >
        {t.segment.backArrow}
      </Link>
    ...
    <main className="min-h-screen overflow-x-hidden bg-surface text-ink">
      <div className="mx-auto grid max-w-2xl gap-8 px-4 py-10 pb-[calc(env(safe-area-inset-bottom)+17rem)] sm:px-6 sm:pb-[calc(env(safe-area-inset-bottom)+15rem)]">
        ...
        <div className="flex flex-wrap items-end justify-between gap-2 border-b border-ink-line/70 pb-5 sm:gap-4">
          <div className="min-w-0">
            <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-accent">
              {t.segment.practiceEyebrow}
            </p>
            <h1 className="mt-2 truncate font-display text-2xl font-semibold tracking-tight sm:text-3xl">
              {segment.title ?? format(t.home.segmentFallbackTitle, { n: segment.index + 1 })}
            </h1>
            <p className="mt-1 text-sm text-ink-muted">
              {format(t.segment.positionInProject, {
                title: project.title,
        ...
        {!audioAvailable ? (
          ...
          >
            <p className="font-medium text-ink">{t.segment.audioMissingTitle}</p>
            <p className="mt-1">
              {t.segment.audioMissingBody}
            </p>
          </div>
        ) : null}
        ...
        <nav className="flex items-center justify-between gap-4 rounded-card border border-ink-line bg-paper px-4 py-3">
          ...
          {prevSegment ? (
            ...
            >
              ...
              <span className="max-w-32 truncate">
                {prevSegment.title ??
                  format(t.home.segmentFallbackTitle, { n: prevSegment.index + 1 })}
              </span>
            </Link>
          ) : prevProjectEntry ? (
            <Link
            ...
            >
              ...
              <span className="shrink-0 font-mono text-[10px] uppercase tracking-[0.14em] text-accent/70">
                {t.common.prev}
              </span>
            </Link>
          ) : (
          ...
          {nextSegment ? (
            ...
            >
              <span className="max-w-32 truncate">
                {nextSegment.title ??
                  format(t.home.segmentFallbackTitle, { n: nextSegment.index + 1 })}
              </span>
              <span className="text-ink-faint">→</span>
            </Link>
          ) : nextProjectEntry ? (
